...
def generateGuessSmallList(possibleWords, lettersGuessed, guesses, startTime):
  ...
  if SHOW_TIMES and DISPLAY_GUESSES:
    print("time to generateGuess (small list): %s" % timeElapsed)
  return bestWord
...
def generateGuess(possibleWords, lettersGuessed, guesses):
  ...
  if len(possibleWords) <= SMALL_WORDS:
    return generateGuessSmallList(possibleWords, lettersGuessed, guesses, startTime)
  ...
  timeElapsed = datetime.now() - startTime
  if SHOW_TIMES and DISPLAY_GUESSES:
    print("time to generateGuess: %s" % timeElapsed)
  return bestWord
####################################################################################################
...
if not HUMAN_INPUT:
  ...
  for i in range(NUM_GAMES):
    ...
    while guess.split(" ")[0] != WORD_TO_GUESS and guesses < MAX_GUESSES + 1:
      guess = getInput(HUMAN_INPUT, WORD_TO_GUESS, guesses, DISPLAY_GUESSES, lettersGuessed, STARTING_GUESS)
      guesses+=1
  ...
  while guesses < MAX_GUESSES:
    guess = getInput(HUMAN_INPUT, WORD_TO_GUESS, guesses, True, lettersGuessed, STARTING_GUESS)
    guesses+=1
    possibleWords = eliminateImpossibleWords(guess, possibleWords)
    lettersGuessed = getLettersGuessed(guess, lettersGuessed)
    print(waitMessage)
