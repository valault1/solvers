...
const getSeedsPath = (size: number) => {
  return `/Users/vault/src/solvers/scripts/boards/seeds${size}.v4.ts`;
};

const currentAppSeedsPath =
  "/Users/vault/src/solvers/src/domains/Queens/boards/allSeeds.ts";
...
export const processNewSeeds = async () => {
  console.log("going to process new seeds");

  const initialSeedsBySize = await getUnusedSeeds();
  ...
  const currentSeedsBySize: { [key: string]: number[] } =
  ...
  const mainPackSeeds: SeedsBySize = {};
  const unusedSeeds: SeedsBySize = {};
  ...
  Object.keys(initialSeedsBySize).forEach((sizeAsStr) => {
    ...
    const currentSeeds: number[] = currentSeedsBySize[sizeAsStr];
    console.log(
    ...
    initialSeeds.forEach((seed) => {
      if (seedIsUsed(seed.value, currentSeeds)) {
        if (mainPackSeeds[sizeAsStr]) {
          mainPackSeeds[sizeAsStr].push(seed);
        } else mainPackSeeds[sizeAsStr] = [seed];
      } else {
        if (unusedSeeds[sizeAsStr]) {
          unusedSeeds[sizeAsStr].push(seed);
        } else unusedSeeds[sizeAsStr] = [seed];
      }
    });
  });
...
export const getUnusedSeeds = async (): Promise<SeedsBySize> => {
  let result: SeedsBySize = {};

  await Promise.all(
    SIDE_LENGTH_OPTIONS.map(async (size) => {
      const fileContent = await fs.readFile(getSeedsPath(size), "utf-8");
      const stats = await fs.stat(getSeedsPath(size));
