...
// represents a card in a deck on moxfield
...
type MoxfieldDeck = {
  ...
  name: string;
  publicUrl: string;
  commanders: MoxfieldDeckCard[];
  mainboard: MoxfieldDeckCard[];
  sideboard: MoxfieldDeckCard[];
  maybeboard: MoxfieldDeckCard[];
};

export const fetchMoxfieldCardsFromDecks = async (
  deckIds: string[]
): Promise<MoxfieldDeck[]> => {
  // query for each deck in parallel
  const promises = deckIds.map((deckId) => {
    return fetch(
      `${CORS_PROXY_URL}https://api2.moxfield.com/v3/decks/all/${deckId}`,
      {
        headers: {
  ...
  const getCardsFromBoard = (board: any): MoxfieldDeckCard[] => {
