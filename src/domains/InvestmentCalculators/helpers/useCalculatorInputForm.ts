...
export const useCalculatorInputForm = (inputFields: FieldMetadata[]) => {
  const initialState = inputFields.reduce<InputFieldsState>((result, nextField) => {
    if (nextField.type === FieldType.NUMBER) result.set(nextField.name, 0);
    else if (nextField.type === FieldType.BOOLEAN) result.set(nextField.name, false);
    else if (nextField.type === FieldType.STRING) result.set(nextField.name, "");
    return result
  }, new Map<string, any>());

  const [inputFieldsState, setInputFieldsState] = React.useState(initialState);

  const handleChange = React.useCallback(({name, value}: {name: string, value: any}) => {
